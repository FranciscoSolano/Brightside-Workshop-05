...
function deleteMarble(color, callback) {
  cmd.get(
    'bright console issue command "F CICSTRN1,MB05 DEL ' + color + '" --cn CUST005',
    function (err, data, stderr) {
      typeof callback === 'function' && callback(err, data, stderr);
    }
  );
...
function getMarbleQuantity(color, callback) {
  ...
  cmd.get(
    'bright jobs submit data-set "CUST005.MARBLES.JCL(MARBDB2)" --rff jobid --rft string',
    function (err, data, stderr) {
      if(err){
        callback(err);
      } else {
...
function updateMarble(color, quantity, callback) {
  cmd.get(
    'bright console issue command "F CICSTRN1,MB05 UPD ' + color + " " + quantity + '" --cn CUST005',
    function (err, data, stderr) {
      typeof callback === 'function' && callback(err, data, stderr);
    }
  );
